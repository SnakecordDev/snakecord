...
.. warning::
    ...
    You must have access to the :attr:`~snakecord.Intents.message_content` intent for the commands extension
...
Any parameter that is accepted by the :class:`~snakecord.ext.commands.Command` constructor can be passed into the decorator. For example, to change
...
Snakecord Converters
++++++++++++++++++++

Working with :ref:`snakecord_api_models` is a fairly common thing when defining commands, as a result the library makes
...
.. code-block:: python3
    ...
    @bot.command()
    async def joined(ctx, *, member: snakecord.Member):
...
A lot of snakecord models work out of the gate as a parameter:
...
.. code-block:: python3
    ...
    @bot.command()
    async def union(ctx, what: typing.Union[snakecord.TextChannel, snakecord.Member]):
        await ctx.send(what)


The ``what`` parameter would either take a :class:`snakecord.TextChannel` converter or a :class:`snakecord.Member` converter.
The way this works is through a left-to-right order. It first attempts to convert the input to a
:class:`snakecord.TextChannel`, and if it fails it tries to convert it to a :class:`snakecord.Member`. If all converters fail,
...
.. code-block:: python3
    ...
    @bot.command()
    async def slap(ctx, members: commands.Greedy[snakecord.Member], *, reason='no reason'):
...
.. code-block:: python3
    ...
    @bot.command()
    async def ban(ctx, members: commands.Greedy[snakecord.Member],
...
.. warning::
    ...
    For example, a signature expecting a :data:`typing.Optional` of a :class:`snakecord.Member` followed by a
...
snakecord.Attachment
^^^^^^^^^^^^^^^^^^^

.. versionadded:: 2.0

The :class:`snakecord.Attachment` converter is a special converter that retrieves an attachment from the uploaded attachments on a message. This converter *does not* look at the message content at all and just the uploaded attachments.
...
.. code-block:: python3
    ...
    @bot.command()
    async def upload(ctx, attachment: snakecord.Attachment):
...
.. code-block:: python3
    ...
    @bot.command()
    async def upload(ctx, attachment: typing.Optional[snakecord.Attachment]):
...
.. code-block:: python3
    ...
    async def upload_many(
        ctx,
        first: snakecord.Attachment,
        second: typing.Optional[snakecord.Attachment],
    ):
        if second is None:
            files = [first.url]
        else:
...
.. code-block:: python3
    ...
    async def upload_many(
        ctx,
        first: snakecord.Attachment,
        remaining: commands.Greedy[snakecord.Attachment],
    ):
        files = [first.url]
...
Note that using a :class:`snakecord.Attachment` converter after a :class:`~ext.commands.Greedy` of :class:`snakecord.Attachment` will always fail since the greedy had already consumed the remaining attachments.
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[snakecord.Member] = commands.flag(name='member', default=lambda ctx: [])
...
.. note::
    ...
    a command line parser. The syntax is mainly inspired by Snakecord's search bar input and as a result
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[snakecord.Member] = commands.flag(name='member')
        reason: str
        days: int = 1
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: Tuple[snakecord.Member, ...]
        reason: str
        days: int = 1
...
.. code-block:: python3
    ...
    @commands.hybrid_command()
    async def ban(ctx, member: snakecord.Member, reason: str, days: int = 1):
        ...
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        member: snakecord.Member = commands.flag(description='The member to ban')
...
Likewise, use of the ``name`` keyword argument allows you to pass renames for the parameter, similar to the :func:`~snakecord.app_commands.rename` decorator.

Note that in hybrid command form, a few annotations are unsupported due to Snakecord limitations:
...
- Late binding behaviour
  ...
  .. code-block:: python3
      ...
      @bot.command()
      async def wave(ctx, to: snakecord.User = commands.parameter(default=lambda ctx: ctx.author)):
  ...
  .. code-block:: python3
      ...
      @bot.command()
      async def wave(ctx, to: snakecord.User = commands.Author):
          await ctx.send(f'Hello {to.mention} :wave:')
...
:func:`.on_command_error` which works like any other event in the :ref:`snakecord-api-events`. This global error handler is
...
handlers that allow us to do just that. First we decorate an error handler function with :meth:`~snakecord.ext.commands.Command.error`:

.. code-block:: python3

    @bot.command()
    async def info(ctx, *, member: snakecord.Member):
...
Due to a Snakecord limitation, slash command groups cannot be invoked directly so the ``fallback``
...
such as :func:`snakecord.app_commands.autocomplete` should be placed below the :func:`~ext.commands.hybrid_command` decorator.
