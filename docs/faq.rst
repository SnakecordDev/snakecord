...
This is a list of Frequently Asked Questions regarding using ``snakecord`` and its extension modules. Feel free to suggest a
...
Example code can be found in the `examples folder <https://github.com/SnakecordDev/snakecord/tree/master/examples>`_
in the repository.
...
Putting both of these pieces of info together, you get the following: ::
    ...
    client = snakecord.Client(activity=snakecord.Game(name='my game'))

    # or, for watching:
    activity = snakecord.Activity(name='my activity', type=snakecord.ActivityType.watching)
    client = snakecord.Client(activity=activity)
...
.. code-block:: python3
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(my_url) as resp:
            ...
            await channel.send(file=snakecord.File(data, 'cool_image.png'))
...
.. code-block:: python3
    ...
    # no ID, do a lookup
    emoji = snakecord.utils.get(guild.emojis, name='LUL')
    if emoji:
        await message.add_reaction(emoji)
...
.. code-block:: python3
    ...
    voice.play(snakecord.FFmpegPCMAudio(url), after=my_after)

How do I run something in the background?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

`Check the background_task.py example. <https://github.com/SnakecordDev/snakecord/blob/master/examples/background_task.py>`_
...
.. code-block:: python3
    ...
    guild = snakecord.utils.get(client.guilds, name='My Server')

    # make sure to check if it's found
    if guild is not None:
        # find a channel by name
        channel = snakecord.utils.get(guild.text_channels, name='cool-channel')
...
Snakecord special-cases uploading an image attachment and using it within an embed so that it will not
...
.. code-block:: python3
    ...
    file = snakecord.File("path/to/my/image.png", filename="image.png")
    embed = snakecord.Embed()
...
This event is now available in the library and Snakecord as of version 2.2. It can be found under :func:`on_audit_log_entry_create`.


Commands Extension
-------------------

Questions regarding ``snakecord.ext.commands`` belong here.
...
Questions regarding :class:`snakecord.ui.View`, :class:`snakecord.ui.Modal`, and their components such as buttons, select menus, etc.
...
1. Attach a message to the :class:`~snakecord.ui.View` using either the return type of :meth:`~abc.Messageable.send` or retrieving it via :meth:`Interaction.original_response`.
...
.. code-block:: python3
    ...
    class MyView(snakecord.ui.View):
        ...
        @snakecord.ui.button(label='Example')
        async def example_button(self, interaction: snakecord.Interaction, button: snakecord.ui.Button):
...
Questions regarding Snakecord's new application commands, commonly known as "slash commands" or "context menu commands".
