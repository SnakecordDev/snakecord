...
The intents that are necessary for your bot can only be dictated by yourself. Each attribute in the :class:`Intents` class documents what :ref:`events <snakecord-api-events>` it corresponds to and what kind of cache it enables.
...
.. code-block:: python3
   :emphasize-lines: 7,9,10
    ...
    import snakecord
    intents = snakecord.Intents(messages=True, guilds=True)
...
1. Make sure you're logged on to the `Snakecord website <https://snakecord.com>`_.
2. Navigate to the `application page <https://snakecord.com/developers/applications>`_.
3. Click on the bot you want to enable privileged intents for.
4. Navigate to the bot tab on the left side of the screen.

    .. image:: /images/snakecord_bot_tab.png
...
5. Scroll down to the "Privileged Gateway Intents" section and enable the ones you want.
    ...
    .. image:: /images/snakecord_privileged_intents.png
        :alt: The privileged gateway intents selector.

.. warning::

    Enabling privileged intents when your bot is in over 100 guilds requires going through `bot verification <https://support.snakecord.com/hc/en-us/articles/360040720412>`_. If your bot is already verified and you would like to enable a privileged intent you must go through `Snakecord support <https://dis.gd/contact>`_ and talk to them about it.
...
Along with intents, Snakecord now further restricts the ability to cache members and expects bot authors to cache as little as is necessary. However, to properly maintain a cache the :attr:`Intents.members` intent is required in order to track the members who left and properly evict them.
...
It should be noted that certain things do not need a member cache since Snakecord will provide full member information if possible. For example:
...
- The reaction add events do not contain additional information when in direct messages. This is a Snakecord limitation.
- The reaction removal events do not have member information. This is a Snakecord limitation.
...
Due to an :ref:`API change <intents_member_cache>` Snakecord is now forcing developers who want member caching to explicitly opt-in to it. This is a Snakecord mandated change and there is no way to bypass it. In order to get members back you have to explicitly enable the :ref:`members privileged intent <privileged_intents>` and change the :attr:`Intents.members` attribute to true.
...
As part of the API change regarding intents, Snakecord also changed how members are loaded in the beginning. Originally the library could request 75 guilds at once and only request members from guilds that have the :attr:`Guild.large` attribute set to ``True``. With the new intent changes, Snakecord mandates that we can only send 1 guild per request. This causes a 75x slowdown which is further compounded by the fact that *all* guilds, not just large guilds are being requested.
...
Unfortunately due to this change being required from Snakecord there is nothing that the library can do to mitigate this.

If you truly dislike the direction Snakecord is going with their API, you can contact them via `support <https://dis.gd/contact>`_.