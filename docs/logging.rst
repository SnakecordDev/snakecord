...
*snakecord* logs errors and debug information via the :mod:`logging` python
module. In order to streamline this process, the library provides default configuration for the ``snakecord`` logger when using :meth:`Client.run`. It is strongly recommended that the logging module is configured, as no errors or warnings will be output if it is not set up.
...
.. code-block:: python3
    ...
    handler = logging.FileHandler(filename='snakecord.log', encoding='utf-8', mode='w')

    # Assume client refers to a snakecord.Client subclass...
...
If you want the logging configuration the library provides to affect all loggers rather than just the ``snakecord`` logger, you can pass ``root_logger=True`` inside :meth:`Client.run`:

.. code-block:: python3

    client.run(token, log_handler=handler, root_logger=True)

If you want to setup logging using the library provided configuration without using :meth:`Client.run`, you can use :func:`snakecord.utils.setup_logging`:

.. code-block:: python3

    import snakecord

    snakecord.utils.setup_logging()

    # or, for example
    snakecord.utils.setup_logging(level=logging.INFO, root=False)
...
.. code-block:: python3
    ...
    logger = logging.getLogger('snakecord')
    logger.setLevel(logging.DEBUG)
    logging.getLogger('snakecord.http').setLevel(logging.INFO)

    handler = logging.handlers.RotatingFileHandler(
        filename='snakecord.log',
        encoding='utf-8',
