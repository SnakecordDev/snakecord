...
Logging on with a user token is against the Snakecord `Terms of Service <https://support.snakecord.com/hc/en-us/articles/115002192352>`_
...
In order to support this, the way snakecord handles the :mod:`asyncio` event loop has changed.
...
This method is called after login but before connecting to the snakecord gateway.
...
.. code-block:: python3
    ...
    client = snakecord.Client(intents=intents)

This change applies to **all** subclasses of :class:`Client`.

- :class:`AutoShardedClient`
- :class:`~snakecord.ext.commands.Bot`
- :class:`~snakecord.ext.commands.AutoShardedBot`


Abstract Base Classes Changes
-------------------------------

:ref:`snakecord_api_abcs` that inherited from :class:`abc.ABCMeta` now inherit from :class:`typing.Protocol`.
...
.. code:: python
    ...
    week_ago = snakecord.utils.utcnow() - datetime.timedelta(days=7)
...
.. code:: python
    ...
    async with aiohttp.ClientSession() as session:
        webhook = snakecord.Webhook.from_url('url-here', adapter=snakecord.AsyncWebhookAdapter(session))
        await webhook.send('Hello World', username='Foo')

    # after
    async with aiohttp.ClientSession() as session:
        webhook = snakecord.Webhook.from_url('url-here', session=session)
        await webhook.send('Hello World', username='Foo')
...
.. code:: python
    ...
    # before
    webhook = snakecord.Webhook.partial(123456, 'token-here', adapter=snakecord.RequestsWebhookAdapter())
    webhook.send('Hello World', username='Foo')

    # after
    webhook = snakecord.SyncWebhook.partial(123456, 'token-here')
    webhook.send('Hello World', username='Foo')
...
Snakecord has changed how their stickers work and as such, sticker support has been reworked.
...
- ``AsyncIterator.get()``
    ...
    .. code:: python
        ...
        # after
        msg = await snakecord.utils.get(channel.history(), author__name='Dave')
...
- ``AsyncIterator.find()``
    ...
    .. code:: python
        ...
        # after
        event = await snakecord.utils.find(predicate, guild.audit_logs())
...
- ``AsyncIterator.chunk()``
    ...
    .. code:: python
        ...
        # after
        async for leader, *users in snakecord.utils.as_chunks(reaction.users(), 3):
            ...
...
Originally, embeds used a special sentinel to denote emptiness or remove an attribute from display. The ``Embed.Empty`` sentinel was made when Snakecord's embed design was in a nebulous state of flux. Since then, the embed design has stabilised and thus the sentinel is seen as legacy.
...
.. code-block:: python
    ...
    # before
    embed = snakecord.Embed(title='foo')
    embed.title = snakecord.Embed.Empty
...
- :meth:`snakecord.ext.tasks.Loop.error`
- :meth:`snakecord.ext.commands.Bot.on_command_error`
...
In the future this may include :class:`StageChannel` when Snakecord implements it.

Removal of ``StoreChannel``
-----------------------------

Snakecord's API has removed store channels as of `March 10th, 2022 <https://support-dev.snakecord.com/hc/en-us/articles/6309018858647>`_. Therefore, the library has removed support for it as well.
...
Due to a breaking API change by Snakecord, :meth:`Guild.bans` no longer returns a list of every ban in the guild but instead is paginated using an asynchronous iterator.
...
- ``VoiceRegion``
    ...
    - This has been marked deprecated by Snakecord and it was usually more or less out of date due to the pace they added them anyway.
...
- ``on_private_channel_create`` event
    ...
    - Snakecord API no longer sends channel create event for DMs.
...
- :attr:`Intents.emojis` is now an alias of :attr:`Intents.emojis_and_stickers`.
    ...
    .. code:: python
        ...
        for name, value in snakecord.Intents.all():
...
- ``created_at`` is no longer part of :class:`abc.Snowflake`.
    ...
    This has been done because Snakecord reuses IDs (snowflakes) of some models in other models.
...
- :attr:`Message.type` is now set to :attr:`MessageType.reply` when a message is a reply.
    ...
    - This is caused by a difference in behavior in the current Snakecord API version.
