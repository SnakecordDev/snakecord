...
See :ref:`snakecord-api-events` for more information. Other removed events include ``on_socket_closed``, ``on_socket_receive``, and ``on_socket_opened``.
...
.. code-block:: python3
    ...
    server.region == snakecord.ServerRegion.us_west
    member.status = snakecord.Status.online
    channel.type == snakecord.ChannelType.text

The main reason for this change was to reduce the use of finicky strings in the API as this
could give users a false sense of power. More information can be found in the :ref:`snakecord-api-enums` page.
...
In earlier versions of snakecord, ``client.run()`` was a blocking call to the main thread
