...
:ref:`models <snakecord_api_models>` instead of requiring a :class:`Client` instance to do any work.
...
put into their respective :ref:`model <snakecord_api_models>`.
...
The types are split into two different :ref:`snakecord_api_abcs`:
...
So to check if something is a guild channel you would do: ::
    ...
    isinstance(channel, snakecord.abc.GuildChannel)

And to check if it's a private channel you would do: ::

    isinstance(channel, snakecord.abc.PrivateChannel)

Of course, if you're looking for only a specific type you can pass that too, e.g. ::

    isinstance(channel, snakecord.TextChannel)
...
- ``Channel.is_private``
    ...
    - Use ``isinstance`` instead with one of the :ref:`snakecord_api_abcs` instead.
    - e.g. ``isinstance(channel, snakecord.abc.GuildChannel)`` will check if it isn't a private channel.
...
- ``Guild.default_channel`` / ``Server.default_channel`` and ``Channel.is_default``
    ...
    - The concept of a default channel was removed from Snakecord.
      See `#329 <https://github.com/hammerandchisel/snakecord-api-docs/pull/329>`_.
...
- :class:`Attachment` to represent a snakecord attachment.
...
file attachments, you must now use a :class:`File` pseudo-namedtuple to upload a single file. ::
    ...
    # after
    await channel.send('Hello', file=snakecord.File('cool.png', 'testing.png'))

This change was to facilitate multiple file uploads: ::

    my_files = [
        snakecord.File('cool.png', 'testing.png'),
        snakecord.File(some_fp, 'cool_filename.png'),
    ]
...
You can also get single elements a la :func:`snakecord.utils.find` or :func:`snakecord.utils.get` via
...
After: ::
    ...
    vc.play(snakecord.FFmpegPCMAudio('testing.mp3'), after=lambda e: print('done', e))
    vc.is_playing()
    vc.pause()
    vc.resume()
    vc.stop()
...
For example, you can add a :class:`PCMVolumeTransformer` to allow changing the volume: ::
    ...
    vc.source = snakecord.PCMVolumeTransformer(vc.source)
...
Usage is as simple as doing: ::
    ...
    client = snakecord.AutoShardedClient()
...
If you want more control over the sharding you can specify ``shard_count`` and ``shard_ids``. ::
    ...
    client = snakecord.AutoShardedClient(shard_count=10)

    # launch specific shard IDs in this process
    client = snakecord.AutoShardedClient(shard_count=10, shard_ids=(1, 2, 5, 6))
...
offline or Snakecord going offline with exponential back-off.
...
as the built-in "snakecord" converters.
