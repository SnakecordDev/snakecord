...
.. |commands| replace:: [:ref:`ext.commands <snakecord_ext_commands>`]
.. |tasks| replace:: [:ref:`ext.tasks <snakecord_ext_tasks>`]
...
- Use cache data first for :attr:`Interaction.channel` instead of API data.
    - This bug usually manifested in incomplete channel objects (e.g. no ``overwrites``) because Snakecord does not provide this data.
...
- Fix crash from Snakecord sending null ``channel_id`` for automod audit logs.
...
- Add setter for :attr:`snakecord.ui.ChannelSelect.channel_types` (:issue:`9068`)
...
- |commands| Respect :attr:`~ext.commands.Command.ignore_extra` for :class:`~snakecord.ext.commands.FlagConverter` keyword-only parameters
...
- Add support for new select types (:issue:`9013`, :issue:`9003`).
    - These are split into separate classes, :class:`~snakecord.ui.ChannelSelect`, :class:`~snakecord.ui.RoleSelect`, :class:`~snakecord.ui.UserSelect`, :class:`~snakecord.ui.MentionableSelect`.
    - The decorator still uses a single function, :meth:`~snakecord.ui.select`. Changing the select type is done by the ``cls`` keyword parameter.
...
- |commands| Add :attr:`GroupCog.group_extras <snakecord.ext.commands.GroupCog.group_extras>` to set :attr:`app_commands.Group.extras` (:issue:`8405`).
- |commands| Add support for NumPy style docstrings for regular commands to set parameter descriptions.
- |commands| Allow :class:`~snakecord.ext.commands.Greedy` to potentially maintain state between calls.
- |commands| Add :meth:`Cog.has_app_command_error_handler <snakecord.ext.commands.Cog.has_app_command_error_handler>` (:issue:`8991`).
- |commands| Allow ``delete_after`` in :meth:`Context.send <snakecord.ext.commands.Context.send>` on ephemeral messages (:issue:`9021`).
...
- |commands| Fix :meth:`Context.from_interaction <snakecord.ext.commands.Context.from_interaction>` ignoring :attr:`~snakecord.ext.commands.Context.command_failed`.
- |commands| Fix :class:`~snakecord.ext.commands.Range` to allow 3.10 Union syntax (:issue:`8446`).
...
- |commands| Fix Python 3.10 union types not working with :class:`commands.Greedy <snakecord.ext.commands.Greedy>`.
...
- Add support for :attr:`MessageReference.fail_if_not_exists` (:issue:`6484`)
    - By default, if the message you're replying to doesn't exist then the API errors out.
      This attribute tells the Snakecord API that it's okay for that message to be missing.

- Add support for Snakecord's new permission serialisation scheme.
...
- Fix user created instances of :class:`CustomActivity` triggering an error (:issue:`4049`)
    - Note that currently, bot users still cannot set a custom activity due to a Snakecord limitation.
...
- Ensure ``snakecord`` package is only run if executed as a script (:issue:`6483`)
...
- :class:`Permission` class methods were updated to match the UI of the Snakecord client (:issue:`6476`)
- ``_`` and ``-`` characters are now stripped when making a new cog using the ``snakecord`` package (:issue:`6313`)
...
- Raise :exc:`SnakecordServerError` when reaching 503s repeatedly (:issue:`6044`)
...
- |commands| :class:`MemberConverter <ext.commands.MemberConverter>` now properly lazily fetches members if not available from cache.
    - This is the same as having ``snakecord.Member`` as the type-hint.
...
This version came with forced breaking changes that Snakecord is requiring all bots to go through on October 7th. It is highly recommended to read the documentation on intents, :ref:`intents_primer`.
...
- Add more concrete exceptions for 500 class errors under :class:`SnakecordServerError` (:issue:`5797`)
...
- The domain name was changed from ``snakecordapp.com`` to ``snakecord.com`` to prepare for the required domain migration
...
- Add support for new audit log types. See :ref:`snakecord-api-audit-logs` for more information. (:issue:`2427`)
...
- Add :attr:`User.system` and :attr:`Profile.system` to know whether a user is an official Snakecord Trust and Safety account.
...
- Due to Snakecord providing :class:`Member` data in mentions, users will now be upgraded to :class:`Member` more often if mentioned.
...
- The rate limiting code now uses millisecond precision to have more granular rate limit handling.
    - Along with that, the rate limiting code now uses Snakecord's response to wait. If you need to use the system clock again for whatever reason, consider passing ``assume_synced_clock`` in :class:`Client`.
...
- Fix issue where :attr:`Guild.premium_subscription_count` would be ``None`` due to a Snakecord bug. (:issue:`2331`, :issue:`2376`).
...
- The Snakecord error code is now shown in the exception message for :exc:`HTTPException`.
...
- Make the output of ``python -m snakecord --version`` a bit more useful.
...
- Add :meth:`Client.change_presence` for changing your status with the new Snakecord API change.
...
- New utility function, :func:`snakecord.utils.get` to simplify retrieval of items based on attributes.
...
- Add :class:`snakecord.Game` to represent any game with custom text to send to :meth:`Client.change_status`.
...
- :meth:`Client.logs_from` now handles cases where limit > 100 to sidestep the snakecord API limitation.
...
- Mentions are now triggered normally. This was changed due to the way snakecord handles it internally.
