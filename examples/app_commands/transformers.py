...
class MyClient(snakecord.Client):
    def __init__(self):
        super().__init__(intents=snakecord.Intents.default())
...
# should behave both when used on snakecord and when you receive it from snakecord.
...
# Other transformers include regular type hints that are supported by snakecord
...
async def channel_info(interaction: snakecord.Interaction, channel: Union[snakecord.VoiceChannel, snakecord.TextChannel]):
    """Shows basic channel info for a text or voice channel."""

    embed = snakecord.Embed(title='Channel Info')
    embed.add_field(name='Name', value=channel.name, inline=True)
    embed.add_field(name='ID', value=channel.id, inline=True)
    embed.add_field(
        name='Type',
        value='Voice' if isinstance(channel, snakecord.VoiceChannel) else 'Text',
        inline=True,
    )
...
# On snakecord, this will show up as two choices, Buy and Sell.
...
async def shop(interaction: snakecord.Interaction, action: Literal['Buy', 'Sell'], item: str):
...
# On snakecord, this will show up as four choices: apple, banana, cherry, and dragonfruit
...
async def fruit(interaction: snakecord.Interaction, fruit: Fruits):
    """Choose a fruit!"""
...
class Point3D(NamedTuple):
    ...
    @classmethod
    async def transform(cls, interaction: snakecord.Interaction, value: str):
        x, y, z = value.split(',')
        return cls(x=int(x.strip()), y=int(y.strip()), z=int(z.strip()))


@client.tree.command()
async def graph3d(interaction: snakecord.Interaction, point: Point3D):
    await interaction.response.send_message(str(point))
