...
@bot.command()
async def userinfo(ctx: commands.Context, user: snakecord.User):
    # In the command signature above, you can see that the `user`
    # parameter is typehinted to `snakecord.User`. This means that
    # during command invocation we will attempt to convert
    # the value passed as `user` to a `snakecord.User` instance.
    # The documentation notes what can be converted, in the case of `snakecord.User`
    ...
    # If the conversion is successful, we will have a `snakecord.User` instance
...
class ChannelOrMemberConverter(commands.Converter):
    async def convert(self, ctx: commands.Context, argument: str):
        ...
        # `snakecord.Member` or `snakecord.TextChannel` instance from the
...
@bot.command()
async def ignore(ctx: commands.Context, target: typing.Union[snakecord.Member, snakecord.TextChannel]):
    ...
    # So, it will attempt to convert whatever is passed to `target` to a `snakecord.Member` instance.
    # If that fails, it will attempt to convert it to a `snakecord.TextChannel` instance.
    # See: https://snakecordpy.readthedocs.io/en/latest/ext/commands/commands.html#typing-union
    ...
    if isinstance(target, snakecord.Member):
        await ctx.send(f'Member found: {target.mention}, adding them to the ignore list.')
    elif isinstance(target, snakecord.TextChannel):  # this could be an `else` but for completeness' sake.
...
async def multiply(ctx: commands.Context, number: int, maybe: bool):
    ...
    # See: https://snakecordpy.readthedocs.io/en/latest/ext/commands/commands.html#bool
