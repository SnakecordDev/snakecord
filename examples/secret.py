...
def create_overwrites(ctx, *objects):
    ...
    A `snakecord.PermissionOverwrite` allows you to determine the permissions
    of an object, whether it be a `snakecord.Role` or a `snakecord.Member`.
    ...
    # for each `snakecord.Role` or `snakecord.Member`.
    overwrites = {obj: snakecord.PermissionOverwrite(view_channel=True) for obj in objects}

    # prevents the default role (@everyone) from viewing the channel
    # if it isn't already allowed to view the channel.
    overwrites.setdefault(ctx.guild.default_role, snakecord.PermissionOverwrite(view_channel=False))

    # makes sure the client is always allowed to view the channel.
    overwrites[ctx.guild.me] = snakecord.PermissionOverwrite(view_channel=True)
...
@secret.command()
@commands.guild_only()
async def text(ctx: commands.Context, name: str, *objects: typing.Union[snakecord.Role, snakecord.Member]):
...
@secret.command()
@commands.guild_only()
async def voice(ctx: commands.Context, name: str, *objects: typing.Union[snakecord.Role, snakecord.Member]):
...
@secret.command()
@commands.guild_only()
async def emoji(ctx: commands.Context, emoji: snakecord.PartialEmoji, *roles: snakecord.Role):
