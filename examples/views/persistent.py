...
class PersistentView(snakecord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @snakecord.ui.button(label='Green', style=snakecord.ButtonStyle.green, custom_id='persistent_view:green')
    async def green(self, interaction: snakecord.Interaction, button: snakecord.ui.Button):
        await interaction.response.send_message('This is green.', ephemeral=True)

    @snakecord.ui.button(label='Red', style=snakecord.ButtonStyle.red, custom_id='persistent_view:red')
    async def red(self, interaction: snakecord.Interaction, button: snakecord.ui.Button):
        await interaction.response.send_message('This is red.', ephemeral=True)

    @snakecord.ui.button(label='Grey', style=snakecord.ButtonStyle.grey, custom_id='persistent_view:grey')
    async def grey(self, interaction: snakecord.Interaction, button: snakecord.ui.Button):
...
class DynamicButton(snakecord.ui.DynamicItem[snakecord.ui.Button], template=r'button:user:(?P<id>[0-9]+)'):
    def __init__(self, user_id: int) -> None:
        super().__init__(
            snakecord.ui.Button(
                label='Do Thing',
                style=snakecord.ButtonStyle.blurple,
    ...
    @classmethod
    async def from_custom_id(cls, interaction: snakecord.Interaction, item: snakecord.ui.Button, match: re.Match[str], /):
        user_id = int(match['id'])
        return cls(user_id)

    async def interaction_check(self, interaction: snakecord.Interaction) -> bool:
        # Only allow the user who created the button to interact with it.
        return interaction.user.id == self.user_id

    async def callback(self, interaction: snakecord.Interaction) -> None:
...
async def dynamic_button(ctx: commands.Context):
    ...
    view = snakecord.ui.View(timeout=None)
